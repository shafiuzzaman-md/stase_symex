...
def write_kernel_stub_header(include_dir: Path):
...
"""
    ...
    print("[\u2713] Wrote include/kernel_stub_defs.h with stub definitions")
...
def comment_out_all_includes_except_stub(file: Path):
    ...
    new_lines = []
    for line in lines:
        if line.strip().startswith("#include") and 'kernel_stub_defs.h' not in line:
...
def main():
    ...
    write_kernel_stub_header(instrumented_src / "include")
